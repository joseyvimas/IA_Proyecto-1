...
bool max_troopers_kill (State estado_actual){
	int max_cant_trooper ;
	//c = 1 + ((x - 1) / y); truco para redondear hacia arriba
	
	max_cant_trooper = ( ( ( 1 + ( ( estado_actual.Time_Left-1 ) /2) )-1) * 3) + (1 + ( ( ( estado_actual.Time_Left-1 ) -1 ) /2) ) -1 ;
	
	if( estado_actual.cant_Troopers > max_cant_trooper ){
		return true;
	}
	else {
		return false;
...
void encolarVecinosV3(State estado_actual, priority_queue<State, vector<State>,greater<vector<State>::value_type> > &cola){
	...
	if(CanForceStormtrooper(hangarActual, estado_actual.pos_actual_X, estado_actual.pos_actual_Y)){
		...
		nuevo_estado->Time_Left = estado_actual.Time_Left-1;  //Disminuimos el tiempo
		int s = cantTrooper(hangarCopia);
		nuevo_estado->cant_Troopers = s;  //Contamos la cantidad de troopers que quedan en el hangar
	...
	//DISPARAR NORTE
	if(canShoot(hangarActual, estado_actual.pos_actual_X, estado_actual.pos_actual_Y, "N", stormX, stormY) && !estado_actual.NoDN){
		//crear estado
		State *nuevo_estado =  new State();
	...
	//DISPARAR SUR
	if(canShoot(hangarActual, estado_actual.pos_actual_X, estado_actual.pos_actual_Y, "S", stormX, stormY) && !estado_actual.NoDS){
		//crear estado
		State *nuevo_estado =  new State();
	...
	//DISPARAR ESTE
	if(canShoot(hangarActual, estado_actual.pos_actual_X, estado_actual.pos_actual_Y, "E", stormX, stormY) && !estado_actual.NoDE){
		//crear estado
		State *nuevo_estado =  new State();
	...
	//DISPARAR OESTE
	if(canShoot(hangarActual, estado_actual.pos_actual_X, estado_actual.pos_actual_Y, "O", stormX, stormY) && !estado_actual.NoDO){
		//crear estado
		State *nuevo_estado =  new State();
...
void busqueda_por_Tiempo (priority_queue<State, vector<State>,greater<vector<State>::value_type> > cola, int i){
	...
	*estado_actual = cola.top();
	cola.pop();
	if( estado_actual->Time_Left <0 /*|| max_troopers_kill(*estado_actual)*/ ){
...
void busqueda_Troopers_Optimizada (priority_queue<State, vector<State>,greater<vector<State>::value_type> > cola, int i){
	...
	*estado_actual = cola.top();
	cola.pop();
	if( estado_actual->Time_Left<0 || ( solucion_Existe && estado_actual->Time_Left<= Tiempo_solucion ) /*|| max_troopers_kill(*estado_actual)*/ ){
	...
	/*if (solucion(hangarActual, estado_actual->Time_Left, estado_actual->solucion)){
		cout << "The Force is Strong in Luke!" << endl;
			for (int x=1; (unsigned)x<Path_solucion.length(); x++){
				cout << Path_solucion[x];
			}
